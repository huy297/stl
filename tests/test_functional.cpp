...
int main() {
    ...
    // compile-time checks on simple integral results.
    // The functors' operator() are not constexpr in the header, so we
    // static_assert on the integral results they are expected to produce.
    static_assert(2 + 3 == 5);
    static_assert(9 - 4 == 5);
